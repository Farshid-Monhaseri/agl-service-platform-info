...
void afv_get(afb_req_t req) {
	...
	if(api_ctx) {
		...
		switch (json_object_get_type(args)) {
			case json_type_null:
				AFB_REQ_DEBUG(req,api_ctx->info,"Info",NULL);
				result = api_ctx->info;
				break;
...
void afv_unsubscribe(afb_req_t req) {
	...
	if(client_ctx) {
		...
		if(event_str && strcmp("monitor-devices",event_str) == 0) {
			afb_req_unsubscribe(req,client_ctx->event_device_changed);
...
void afv_subscribe(afb_req_t req) {
	if(afb_req_is_valid(req)) {
		...
		if(event_str && strcmp("monitor-devices",event_str) == 0) {
			if(!afb_req_context_get(req)) {
				if(pinfo_device_monitor(req) == PINFO_OK) {
					AFB_REQ_DEBUG(req,"context is ok.");
					afb_req_success(req,NULL,NULL);
				} else {
		...
		afb_req_fail(req,"failed","Invalid request");
	}
}
